...
if [ "$command" = 'get' ]; then
	...
	if [ "result" = '' ]; then
		echo "EXEC UserEvent DB_$dbFamily|\"сhannel:$chan\"|\"key:$key\"|\"val:\"";
	else
		echo "EXEC UserEvent DB_$dbFamily|\"сhannel:$chan\"|\"key:$key\"|\"val:$result\"";
	fi;
	...
	if [ "$result" = 'result=1' ]; then
		echo "EXEC UserEvent DB_$dbFamily|\"сhannel:$chan\"|\"key:$key\"|\"val:$val\"";
	else
		echo "EXEC UserEvent Error_data_put_$dbFamily|\"сhannel:$chan\"|\"key:$key\"|\"val:$val\"";
	fi;
	#
	...
	while [ $i -le $kol ]; do        
		...
		echo "EXEC UserEvent From$dbFamily|\"Channel:$chan\"|\"Lines:$result\"";
		read RESPONSE;
	done;
fi;